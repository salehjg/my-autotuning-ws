...
// Declare specialization constant globally
constexpr sycl::specialization_id<std::size_t> tile_spec_id;

int main(int argc, char** argv) {
    std::size_t N = 256;     // Matrix size
    std::size_t TILE = 16;   // Tile size
    
    if (argc >= 2) N = std::stoul(argv[1]);
    if (argc >= 3) TILE = std::stoul(argv[2]);
    ...
    // Submit kernel with specialization constant
    q.submit([&](sycl::handler& h) {
        // Set the specialization constant value
        h.set_specialization_constant<tile_spec_id>(TILE);
        ...
        h.parallel_for(
            ...
            [=](sycl::nd_item<2> it, sycl::kernel_handler kh) {
                // Retrieve specialization constant in kernel
                const std::size_t tile = kh.get_specialization_constant<tile_spec_id>();
